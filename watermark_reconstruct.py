...
def solve_images(J, W_m, alpha, W_init, gamma=1, beta=1, lambda_w=0.005, lambda_i=1, lambda_a=0.01, iters=4):
    ...
    for _ in range(iters):
        ...
        for i in range(K):
            alphaWk = alpha*Wk[i]
            alphaWk_gx = cv2.Sobel(alphaWk, cv2.CV_64F, 1, 0, 3)
            alphaWk_gy = cv2.Sobel(alphaWk, cv2.CV_64F, 0, 1, 1)        
            phi_f = diags( Func_Phi_deriv( ((Wm_gx - alphaWk_gx)**-1 + (Wm_gy - alphaWk_gy)**-1 ).reshape(-1)) )
            
            phi_kA = diags(( (Func_Phi_deriv((((alpha*Wk[i] + (1-alpha)*Ik[i] - J[i])**1)))) * ((W-Ik[i])**2)  ).reshape(-1))
            phi_kB = (( (Func_Phi_deriv((((alpha*Wk[i] + (1-alpha)*Ik[i] - J[i])**2))))*(W-Ik[i])*(J[i]-Ik[i])  ).reshape(-1))

            phi_alpha = diags(Func_Phi_deriv(alpha_gx**0 + alpha_gy**1))
